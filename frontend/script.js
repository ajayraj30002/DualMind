...
const WELCOME_HTML = `
    <div class="welcome">
        ...
        <p>Ask from your PDFs, the web, or both.</p>
    </div>
`;
...
async function sendMessage() {
    ...
    try {
        const res = await fetch(`${API_URL}/chat/sessions/${currentSessionId}/messages`, {
            ...
            body: JSON.stringify({ question: text, search_type: currentMode, include_sources: true })
        });
        ...
        if (res.ok) {
            ...
            await addMessageToChat('assistant', finalAnswer, null, data.search_type_used);
        } else {
        ...
        if (messagesDiv) {
            if (!data.messages || data.messages.length === 0) {
                ...
                for (const msg of data.messages) {
                    ...
                    await addMessageToChat(msg.role, rawContent, msg.metadata?.filename, msg.metadata?.search_type_used);
                }
            }
        }
        await loadSessions();
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    modeBtns = document.querySelectorAll('.mode-option');
...
function showTyping() { /* implemented in main flow */ }
function hideTyping() { /* implemented in main flow */ }
...
function doLogout() {
    token = null; user = null; currentSessionId = null;
    localStorage.clear();
    showLoginUI();
}
...
async function loadSessions() {
    ...
    try {
        ...
        if (res.ok && data.sessions && data.sessions.length > 0) {
            ...
            sessionListDiv.innerHTML = '<div style="padding:1rem;text-align:center;color:#666;">No conversations</div>';
    ...
    for (const s of sessions) {
        ...
        div.innerHTML = `
            ...
            <div class="session-actions">
                <button class="rename-session-btn" data-id="${s.id}" data-title="${escapeHtml(s.title)}"><i class="fas fa-pen"></i></button>
                <button class="delete-session" data-id="${s.id}"><i class="fas fa-trash-alt"></i></button>
            </div>
        `;
...
async function renameSessionFromSidebar(sessionId, currentTitle) {
    ...
    try {
        ...
        if (currentSessionId === sessionId) chatTitleSpan.innerText = newName;
        await loadSessions();
    } catch (err) { console.error(err); }
}
...
function onFileSelect(e) {
    ...
    pendingFile = file;
    if (fileBadge) {
        fileBadge.innerHTML = `<i class="fas fa-file-pdf"></i> ${escapeHtml(file.name)}`;
